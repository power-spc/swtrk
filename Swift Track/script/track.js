...
document.querySelector('.tracking-form').addEventListener('submit', function(e) {
  ...
  const input = this.querySelector('input').value.trim().toUpperCase();
  ...
    BS: "Bahamas",BH: "Bahrain",
    ...
    SB: "Solomon Islands",
    SO: "Somalia",
    ZA: "South Africa",SS: "South Sudan",
    ES: "Spain",
    LK: "Sri Lanka",
    SD: "Sudan",
    SR: "Suriname",
  ...
  const country = countryCodes[countryCode] || 'Unknown Location';

  const statuses = ['In Transit', 'Out for Delivery', 'Delivered', 'Delayed'];
  const status = statuses[Math.floor(Math.random() * statuses.length)];

  result.style.color = '#fff';
  result.textContent = Tracking Number: input | Location:{country} | Status: ${status};
});
```
